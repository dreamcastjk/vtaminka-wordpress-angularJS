...
class ajaxApi implements interfaceApi{
    ...
    //список товаров на главную страницу
    public static function getProductListAction(){

        $limit = filter_input(INPUT_POST,'limit',FILTER_SANITIZE_NUMBER_INT);
        $offset = 0;

        //получаем список товаров
        $productsList = get_posts([
            'numberposts' => $limit,
            'post_type' => 'goods'
        ]);
        ...
        foreach ($productsList as $singleProduct):
            ...
            //название продукта
            $productTitle = $singleProduct->post_title;

            //цена продукта
            $productPrice = get_post_meta($productId, 'price', true);

            //изображение продукта
            $productImageID = get_post_thumbnail_id($productId);
            $productImage = wp_get_attachment_image_src($productImageID, 'full');
    ...
    //вывод категорий товаров для главной страницы
    public static function getCategoryListAction(){

        $categoriesList = get_terms( 'vtaminkataxonomy' , [
            'orderby'           => 'name',
            'order'             => 'ASC',
            'hide_empty'        => false,
            'exclude'           => array(),
    ...
    //вывод товаров пока категории
    public static function getProductsByCategoryAction(){

        // получаем название категории
        $categoryName = filter_input(INPUT_POST,'categoryName',FILTER_SANITIZE_STRING);

        // получаем список товаров по категории
        $productsByCategoryList = query_posts(
            [
        ...
        foreach ($productsByCategoryList as $productByCategoty):

            $productID = $productByCategoty->ID; //id продукта
            $productTitle = $productByCategoty->post_title; // назваение продукта
    ...
    //оформление зказа
    public static function purchaseOrderAction(){

        // информация о заказе покупателя
        $customerOrder =  filter_input(INPUT_POST, 'order');

        $customerOrder = json_decode($customerOrder);

        // список товаров покупателя
        $customerProducts = $customerOrder->customerProducts->products;

        // детальная информация о заказе покупателя
        $orderDetailString = "";
        foreach ($customerProducts as $product):

            $customerName = $customerOrder->customer->customerName; // имя пользователя
            $productID = $product->ProductID;                       // ID
            $productTitle = $product->ProductTitle;                 // название
            $productPrice = $product->ProductPrice;                 // цена
            $productAmount = $product->amount;                      // количество

            $orderDetailString .= "Customer Name: " . $customerName . "<br/>";
            $orderDetailString .= "Product ID: " . $productID . "<br/>";
            $orderDetailString .= "Product Title: " . $productTitle . "<br/>";
            $orderDetailString .= "Product Price: " . $productPrice . "<br/>";
            $orderDetailString .= "Product Amount: " . $productAmount . "<br/>";

        endforeach;

        // добавление заказа в базу
        $postID = wp_insert_post([
            'post_title' => $customerOrder->customer->customerName,
            'post_content' => $orderDetailString,
            'post_type' => 'orders',
            'post_status'   => 'publish',
            'post_author'   => 1,
        ]);

        // в случае неудачи создания записи о новом заказе
        ...
        update_post_meta( $postID , 'email' , $customerEmail );
        update_post_meta( $postID , 'phone' , $customerPhoneNumber );
        update_post_meta( $postID , 'address' , $customerDeliveryAddress );

        // данные по оплате
        $customerPayment = $customerOrder->customerPayment;

        $customerCardNumber = $customerPayment->customerCardNumber;
        $customerCardMonth = $customerPayment->customerCardMonth;
        $customerCardYear = $customerPayment->customerCardYear;
        $customerCardCvvCode = $customerPayment->customerCardCvvCode;

        update_post_meta( $postID , 'card' , $customerCardNumber );
        update_post_meta( $postID , 'month' , $customerCardMonth );
        update_post_meta( $postID , 'year' , $customerCardYear );
        update_post_meta( $postID , 'cvv' , $customerCardCvvCode );
